...
class binance_strategy(IStrategy):
    minimal_roi = {
        "0": 10
    }
    ...
    # Corrected startup candle count to match the longest indicator period (EWO uses 200)
    ...
    # Your hyperopt parameters remain unchanged...
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        # --- CRITICAL FIX: Enforce consistent data types for TA-Lib ---
        # TA-Lib functions are sensitive to input types. We cast to `float` to ensure consistency.
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        conditions = []
        # --- FIX: Initialize with a non-empty string for consistent data type ---
    ...
    def custom_stoploss(self, pair: str, trade: Trade, current_time: datetime, current_rate: float,
        ...
        # ... (Your custom stoploss logic remains unchanged) ...
    ...
    def custom_exit(self, pair: str, trade: Trade, current_time: datetime, current_rate: float,
        ...
        # ... (Your custom exit logic remains unchanged) ...
        dataframe, _ = self.dp.get_analyzed_dataframe(pair, self.timeframe)
        current_candle = dataframe.iloc[-1].squeeze()
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        # Also ensure the exit column is initialized cleanly
